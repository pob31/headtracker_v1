...
A head-worn IMU unit streams orientation quaternions over a proprietary 2.4 GHz radio link
...
```
 head-worn unit                        USB dongle                  host PC
┌──────────────────────────┐   ESB    ┌─────────────────┐  CDC   ┌─────────────────────┐
│ LSM6DS3TR-C ─► VQF fusion│ 2.4 GHz  │ ESB RX ─► framer│  USB   │ spatial audio app   │
│ 416 Hz IMU     (6DoF)    │ ═══════► │                 │ ─────► │ (WFS-DIY / XOA /    │
│ XIAO nRF52840 Sense      │ ◄─ ACK ─ │ XIAO nRF52840   │ ◄───── │  Tight-WFS)         │
└──────────────────────────┘ commands └─────────────────┘  cmds  └─────────────────────┘
        quaternions @ ~208 Hz                            COBS-framed binary protocol
```
...
host/
  ...
  tools/htmon/     # CLI monitor: live Euler/quat, rates, loss stats, recenter key
...
reference library and CLI monitor are planned under `host/`.
